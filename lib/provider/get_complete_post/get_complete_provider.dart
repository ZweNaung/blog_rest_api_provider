...
class GetCompleteNotifier extends ChangeNotifier{
  ...
  Future<void> getCompletePost({required int id})async{
      getCompletePostState = GetCompletePostLoading();
      notifyListeners();

      try {
        GetOnePostResponse getOnePostResponse = await _blogApiService
            .getOnePost(id);
        getCompletePostState = GetCompletePostSuccess(getOnePostResponse);
        notifyListeners();
      }catch(e){
        getCompletePostState = GetCompletePostFailed("Something Wrong in GetComplete Post");
        notifyListeners();
      }
